...
* Replace the use of command-line utilities for typical post-alignment processing, e.g. `bedtools` and `deeptools`
...
* Process and import bam files quickly and easily; or import existing bedGraph or bigWig files
* Count reads by regions of interest
* Counts reads at positions within regions of interest, at single-base resolution or in larger bins, and generate count matrices for heatmapping
...
* UMI deduplification
* Separating and counting spike-in reads
* Generating normalization factors, including several distinct and meaningful methods for calculating spike-in normalization factors
...
* Support for region black-listing in generating signal matrices and in meta-profiling, and increased support in DESeq2 functions
...
ps_reads <- import_bam(bfile, mapq = 20, revcomp = TRUE)
ps_reads
```
...
The 3' end of a mapped PRO-seq read gives the position of an RNA polymerase at basepair-resolution. The `import_bam` function also includes options for selecting specific bases of each read, which are applied after reverse-complementing:
