...
By default, many BRGenomics functions will use multicore processing as implemented in the `parallel` package, and the default is to use all available cores (found using `detectCores()`). 

However, parallel processing is not available on Windows. To maintain compatability, all code in this vignette as well as the example code in the documentation is to use a single core, i.e. `ncores = 1`.
...
getCountsByRegions(ps_multi, txs_dm6_chr4[1:5], field = "ps2", ncores = 1)

# if no field is given, most functions will default to using all fields
getCountsByRegions(ps_multi, txs_dm6_chr4[1:5], ncores = 1)
```
...
```{r}
merge_ps <- mergeGRangesData(ps1, ps2, ps3, ncores = 1)
merge_ps
```
