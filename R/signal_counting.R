...
#' \code{regions.gr}. \code{getCountsByRegions} is written to calculate
#' \emph{readcounts} overlapping each region, while \code{getCoverageByRegions}
#' is written to calculate "coverage signal" (see details below).
#'
...
#'
#' @section getCountsByPositions: This function is designed to work with data in
#'   which each range represents one type of molecule, whether it's a single
#'   base (e.g. the 5' ends, 3' ends, or centers of reads) or entire reads (i.e.
#'   paired 5' and 3' ends of reads).
#'
...
#'   the same signal information. \code{getCountsbyRegions} does \emph{not}
#'   expand run-length compressed coverage objects.
#'
...
#'
#' @section getCoverageByPositions: Unlike
#'   \code{\link[BRGenomics:getCountsByRegions]{getCountsByRegions}}, this
#'   function assumes that ranges in \code{dataset.gr} (>1 bp) are compressed
#'   representations of multiple adjacent positions that contain the same
#'   signal. This type of representation is typical of "coverage" objects,
...
#'
#'   As an example, \code{getCoverageByRegions} treats a range of covering 10 bp
#'   with a score of 2 as 20 signal counts, i.e. there are 10 adjacent positions
#'   each with a signal of 2.
#'
...
#'   the data is whole-read coverage. In this case, the "coverage signal" is
#'   determined both by the read counts as well as read lengths.
#'
#' @author Mike DeBerardine
#' @seealso
#'   \code{\link[BRGenomics:getCountsByPositions]{
#'   getCountsByPositions/getCoverageByPositions}}
#'
#' @export
...
#' \code{getCountsByPositions} and \code{getCoverageByPositions}, see the
#' documentation for \code{\link[BRGenomics:getCountsByRegions]{
...
#' @author Mike DeBerardine
#' @seealso
#'   \code{\link[BRGenomics:getCountsByRegions]{
#'   getCountsByRegions/getCoverageByRegions}}
#'
#' @export
#' @importFrom parallel detectCores mcMap mclapply
...
getPausingIndices <- function(dataset.gr, promoters.gr, genebodies.gr,
    ...
    FUN <- if (expand_ranges) getCoverageByRegions else getCountsByRegions
    counts_pr <- FUN(dataset.gr, promoters.gr, field = field, ncores = ncores)
    counts_gb <- FUN(dataset.gr, genebodies.gr, field = field, ncores = ncores)
...
#' @importFrom parallel detectCores mcMap
#' @export
getCoverageByRegions <- function(dataset.gr, regions.gr, field = "score",
                                 NF = NULL, blacklist = NULL, melt = FALSE,
                                 region_names = NULL, ncores = detectCores()) {
...
.blacklist_cvg <- function(dataset.gr, blacklist, ncores) {
    ...
    whitelist <- gaps(blacklist, end = seqlengths(dat))
    whitelist <- whitelist[strand(whitelist) != "*"]
    pintersect(findOverlapPairs(dat, whitelist), drop.nohit.ranges = TRUE)
}
...
#' @export
getCoverageByPositions <- function(dataset.gr, regions.gr,
                                   binsize = 1, FUN = sum,
                                   simplify.multi.widths = c("error", "list",
                                                             "pad 0", "pad NA"),
                                   field = "score", NF = NULL, blacklist = NULL,
...
.split_cvbp <- function(p_int, x, regions.gr, binsize, FUN, smw, field,
                        NF, melt, blacklist, xy.bl) {
