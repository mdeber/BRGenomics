...
genebodies <- function(genelist,
    ...
    starts <- start(resize(genelist, 1, fix = fix_start))
    ends <- start(resize(genelist, 1, fix = fix_end))

    idx_p <- which(as.character(strand(genelist)) == "+")
    idx_m <- which(as.character(strand(genelist)) == "-")

    starts[idx_p] <- starts + start
    starts[idx_m] <- starts - start
    ends[idx_p] <- ends + end
    ends[idx_m] <- ends - end
