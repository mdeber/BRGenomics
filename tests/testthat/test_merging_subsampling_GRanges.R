...
testthat("Subsampling produces GRanges", {
    expect_is(ps_tenth, "GRanges")
})

testthat("Subsampling gives correct signal count", {
    expect_equal(floor(sum(score(PROseq))), sum(score(ps_tenth)))
})

testthat("Subsamplied reproduced using set.seed", {
...
testthat("Merging single dataset produces warning", {
    expect_warning(mergeGRangesData(PROseq))
})

testthat("Merging is non-destructive", {
...
testthat("Merging non-single-width GRanges produces warning", {
    expect_warning(mergeGRangesData(PROseq, resize(ps_ranges, 2)))
})

testthat("Merging sums signals in overlaps", {
...
testthat("Merging concatenates non-overlapping ranges", {
